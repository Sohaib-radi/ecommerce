...
export default function EcommerceProductItemCountDown({ product, color = 'primary', sx }: Props) {
  ...
  return (
    <Link component={RouterLink} href={`${paths.eCommerce.product}/${product.id}`} color="inherit" underline="none">
      <Stack
        spacing={3}
        sx={{
          p: 3,
      ...
      >
        ...
        <Stack spacing={1} sx={{ textAlign: 'center' }}>
          ...
          <Typography variant="h5">{`فقط  ${fCurrency(product.price)}`}</Typography>
        </Stack>

        <ProductCountdownBlock expired={add(new Date(), { days: 1  + parseInt(product.id, 10), hours: 8 })} />
      </Stack>
    </Link>
  );
}
